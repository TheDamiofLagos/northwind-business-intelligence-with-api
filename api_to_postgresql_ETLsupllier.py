...
API_ENDPOINT = "https://northwind.now.sh/api/suppliers"
...
def store_data_in_postgresql(data):
    ...
    for entry in data:
        ...
        address_json = json.dumps(entry.get('address', {}))

        # Check if 'contactTitle' key is present
        contact_title = entry.get('contactTitle', None)

        insert_query = '''
            INSERT INTO supplier_table (id, companyName, contactName, contactTitle, address)
            VALUES (%s, %s, %s, %s, %s)
        '''
